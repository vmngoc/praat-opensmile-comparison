...
praat_dir = os.path.join(data_dir,'Praat/shs')
os_dir = os.path.join(data_dir,'openSMILE/shs')
saved_dir = os.path.join(current_dir, 'shs')
...
for filepath in f:
	...
	opensmile_data = np.genfromtxt(os.path.join(os_dir, soundname+'-prosodyShs.csv'), delimiter = ';', skiprows=1, usecols=(1,2), unpack=True)
	
	# process openSMILE data so that all values under 50 are eliminated (since minimum setting for f0 in Praat is 50)
	opensmile_data = opensmile_data[:,opensmile_data[1]>=50]
